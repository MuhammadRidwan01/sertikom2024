...
'''
    ...
    post {
        success {
            ...
            // Mengirim status "success" ke GitHub saat build berhasil dengan deskripsi lebih informatif
            script {
                sh """
                    curl -XPOST -H "Authorization: token ${GITHUB_TOKEN}" \
                    https://api.github.com/repos/${GITHUB_ORG}/${GITHUB_REPO}/statuses/\$(git rev-parse HEAD) \
        ...
        failure {
            ...
            // Mengirim status "failure" ke GitHub saat build gagal dengan deskripsi lebih informatif
            script {
